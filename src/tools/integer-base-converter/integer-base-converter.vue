...
<template>
  <div>
    <c-card>
      <c-input-text v-model:value="input" label="输入数字" placeholder="请在此输入数字（如：42）" label-position="left" label-width="110px" mb-2 label-align="right" />

      <n-form-item label="输入进制" label-placement="left" label-width="110" :show-feedback="false">
        <n-input-number v-model:value="inputBase" max="64" min="2" placeholder="请输入进制（如：10）" w-full />
      </n-form-item>
      ...
      <div flex items-baseline>
        ...
        <InputCopyable
          ...
          :placeholder="`${outputBase} 进制结果将显示在此...`"
        />
      </div>
    </c-card>
  </div>
