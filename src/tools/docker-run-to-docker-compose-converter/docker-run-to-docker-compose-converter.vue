...
<template>
  <div>
    <c-input-text
      ...
      raw-text
      monospace
      placeholder="请输入要转换的 docker run 命令..."
      rows="3"
    />
    ...
    <div v-if="notComposable.length > 0">
      <n-alert title="以下选项无法转换为 docker-compose" type="info" mt-5>
        <ul>
    ...
    <div v-if="notImplemented.length > 0">
      <n-alert
        title="以下选项尚未实现，因此未被转换为 docker-compose"
        type="warning"
        mt-5
      >
        <ul>
    ...
    <div v-if="errors.length > 0">
      <n-alert title="发生了以下错误" type="error" mt-5>
        <ul>
